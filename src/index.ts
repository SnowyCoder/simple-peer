/*! pear2pear. MIT License. Lorenzo Rossi */
/*  previously forked from simple-peer, Feross Aboukhadijeh <https://feross.org/opensource> */
...
function errCode(err: Error, code: string): PeerError {
...
export interface PeerConfig {
  ...
  sdpTransform?: (this: Peer, sdp: string) => string;
...
export class Peer extends EventEmitter {
  ...
  readonly answerOptions;
  readonly sdpTransform: (this: Peer, sdp: string) => string;
  ...
  private _iceCompleteTimer: any;
  private _pendingCandidates: Array<RTCIceCandidate>;
  ...
  private _sendersAwaitingStable: Array<RTCRtpSender>;
  private _closingInterval: any;
  private _interval: any;
  private _senderMap: Map<MediaStreamTrack, Map<MediaStream, RTCRtpSender>>;
  private _remoteTracks: Array<{
    track: MediaStreamTrack,
    stream: MediaStream,
  }>;
  private _remoteStreams: Array<MediaStream>;
  private _chunk: SimplePeerData | null;
  private _cb: ((err?: Error) => void) | null;

  constructor(opts?: PeerConfig) {
    super();
    opts = opts ?? {};
    ...
    try {
      ...
      this._destroy(errCode(err, 'ERR_PC_CONSTRUCTOR'));
      return;
    }
    ...
    if (typeof (this._pc as any).peerIdentity === 'object') {
      (this._pc as any).peerIdentity.catch((err) => {
        this._destroy(errCode(err, 'ERR_PC_PEER_IDENTITY'));
      });
    }
  ...
  signal(data: SignalData): void {
    ...
    if ('renegotiate' in data && this.initiator) {
      used = true;
      this._debug('got request to renegotiate');
      this._needsNegotiation();
    }
    if ('transceiverRequest' in data && this.initiator) {
      used = true;
    ...
    if ('sdp' in data) {
      ...
      this._pc.setRemoteDescription(new RTCSessionDescription(data))
        ...
        .catch((err) => {
          this._destroy(errCode(err, 'ERR_SET_REMOTE_DESCRIPTION'));
        });
    }
    if (!used) {
      this._destroy(errCode(new Error('signal() called with invalid signal data'), 'ERR_SIGNALING'));
    }
  }
    ...
    this._pc.addIceCandidate(iceCandidateObj)
      .catch((err) => {
        if (!iceCandidateObj.address || iceCandidateObj.address.endsWith('.local')) {
          ...
          this._destroy(errCode(err, 'ERR_ADD_ICE_CANDIDATE'));
        }
      });
  }
   ...
   */
  send(chunk: string | Blob | ArrayBuffer | ArrayBufferView): void {
    if (this.destroying) return;
    if (this.destroyed) throw errCode(new Error('cannot send after peer is destroyed'), 'ERR_DESTROYED');
    this._channel.send(chunk as any);
  }
   ...
   * @param {Object} init
   */
  addTransceiver(kind: string | MediaStreamTrack, init?: RTCRtpTransceiverInit): void {
    if (this.destroying) return;
    ...
    if (this.initiator) {
      try {
        ...
        this._destroy(errCode(err, 'ERR_ADD_TRANSCEIVER'));
      }
    } else {
  ...
  replaceTrack(oldTrack: MediaStreamTrack, newTrack: MediaStreamTrack, stream: MediaStream): void {
    ...
    if (sender.replaceTrack != null) {
      ...
      this._destroy(errCode(new Error('replaceTrack is not supported in this browser'), 'ERR_UNSUPPORTED_REPLACETRACK'));
    }
  }
  ...
  removeTrack(track: MediaStreamTrack, stream: MediaStream): void {
    ...
    try {
      ...
      if (err.name === 'NS_ERROR_UNEXPECTED') {
        ...
        this._destroy(errCode(err, 'ERR_REMOVE_TRACK'));
      }
    }
    this._needsNegotiation();
  }
  ...
  _setupData(event: { channel?: RTCDataChannel }): void {
    if (!event.channel) {
      // In some situations `pc.createDataChannel()` returns `undefined` (in wrtc),
      // which is invalid behavior. Handle it gracefully.
      // See: https://github.com/feross/simple-peer/issues/163
      this._destroy(errCode(new Error('Data channel event is missing `channel` property'), 'ERR_DATA_CHANNEL'));
      return;
    }
    ...
    this._channel.onerror = (event) => {
      ...
      this._destroy(errCode(err, 'ERR_DATA_CHANNEL'));
    };
  ...
  write(chunk: SimplePeerData, cb: (err?: Error) => void): void {
    if (this.destroyed) {
      cb(errCode(new Error('cannot write after peer is destroyed'), 'ERR_DATA_CHANNEL'));
      return;
    }
  ...
  _createOffer(): void {
    ...
    this._pc.createOffer(this.offerOptions)
      .then((offer) => {
        ...
        const onError = (err) => {
          this._destroy(errCode(err, 'ERR_SET_LOCAL_DESCRIPTION'));
        };
      ...
      .catch((err) => {
        this._destroy(errCode(err, 'ERR_CREATE_OFFER'));
      });
  }
  ...
  _createAnswer(): void {
    ...
    this._pc.createAnswer(this.answerOptions)
      ...
      .catch((err) => {
        this._destroy(errCode(err, 'ERR_CREATE_ANSWER'));
      });
  }
  ...
  _onConnectionStateChange(): void {
    ...
    if (this._pc.connectionState === 'failed') {
      this._destroy(errCode(new Error('Connection failed.'), 'ERR_CONNECTION_FAILURE'));
    }
  }
  ...
  _onIceStateChange(): void {
    ...
    if (iceConnectionState === 'failed') {
      this._destroy(errCode(new Error('Ice connection failed.'), 'ERR_ICE_CONNECTION_FAILURE'));
    }
    if (iceConnectionState === 'closed') {
      this._destroy(errCode(new Error('Ice connection closed.'), 'ERR_ICE_CONNECTION_CLOSED'));
    }
  }

  getStats(cb: (err?: Error, reports?: Array<any>) => void): void {
